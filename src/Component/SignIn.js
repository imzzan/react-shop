...
import { saveIdentitas } from '../features/identitasSlice';
...
const SignIn = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        await dispatch(saveIdentitas({nama, number, alamat}));
        navigate('/acount')
    ...
    return (
        <>
            <Tsparticles/>
            <motion.h1 variants={LoginAnimation} initial='hidden' animate='visible' className=' text-center text-white mt-10 text-2xl font-bold'>Sign In</motion.h1>
            <motion.form onSubmit={handleSubmit} variants={LoginAnimation} initial='hidden' animate='visible' className=' mx-auto bg-white p-3 md:w-1/2 rounded-xl mt-10 w-[90%]'>
                ...
                <button className=' block mx-auto bg-blue-500 text-white px-3 mt-5 rounded-lg'>Submit</button>
            </motion.form>
        </>
